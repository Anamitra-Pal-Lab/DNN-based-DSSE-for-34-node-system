...
%         all_loads_kw_new(:,i) = sum(use(chosen_hour_index,rand_load_index),2)+shifting_factor;
        ...
        Kernel_Model = truncate(Kernel_Model,0.001,1000);
        all_loads_kw_KDE(:,i) = random(Kernel_Model,[mc_samples,1]);
        all_loads_kvar_KDE(:,i) = all_loads_kw_KDE(:,i)*tan(0.3176*rand(1,1));   % assigning random power factor betweeen 0.95 and 1
        all_loads_kw_minutely(:,i) = sum(use(chosen_hour_index,rand_load_index),2)+shifting_factor;
        all_loads_kvar_minutely(:,i) = all_loads_kw_minutely(:,i)*tan(0.3176*rand(1,1));   % assigning random power factor betweeen 0.95 and 1
    else
        scaling_factor = all_loads_kw(i,1)/max(sum(quarterly_smart_meter_data_use,2));
%         all_loads_kw_new(:,i) = sum(use(chosen_hour_index,rand_load_index),2)*scaling_factor;
        ...
        all_loads_kw_KDE(:,i) = random(Kernel_Model,[mc_samples,1]);
        all_loads_kvar_KDE(:,i) = all_loads_kw_KDE(:,i)*tan(0.3176*rand(1,1));  % assigning random power factor betweeen 0.95 and 1
        all_loads_kw_minutely(:,i) = sum(use(chosen_hour_index,rand_load_index),2)*scaling_factor;
        all_loads_kvar_minutely(:,i) = all_loads_kw_minutely(:,i)*tan(0.3176*rand(1,1));   % assigning random power factor betweeen 0.95 and 1
    end
    random_index_each_load(i,:) = rand_load_index;
end
all_loads_kw_new = [all_loads_kw_KDE;all_loads_kw_minutely];
all_loads_kvar_new = [all_loads_kvar_KDE;all_loads_kvar_minutely];

% close all
% index_load = 30;
% histogram(all_loads_kw_KDE(:,index_load))
% figure
% histogram(all_loads_kw_minutely(:,index_load))

% close all
% 
% histogram(sum(all_loads_kw_KDE,2))
% figure
% histogram(sum(all_loads_kw_minutely,2))
...
%    shifting_factor = all_pvs_kw(i,1) - max(sum(solar(chosen_hour_index,random_index_each_load(ib(counter),:)),2));
   ...
   for j = 1:15:length(dates_chosen_hour)
    ...
    all_pvs_kw_KDE(:,i) = random(Kernel_Model,[mc_samples,1]);
    all_pvs_kw_minutely(:,i) = sum(solar(chosen_hour_index,random_index_each_load(ib(counter),:)),2)+shifting_factor;
    counter = counter +1;
end
all_pvs_kw_new = [all_pvs_kw_KDE;all_pvs_kw_minutely];
